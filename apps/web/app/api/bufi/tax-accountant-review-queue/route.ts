...
import {
  AccountantReviewQueueReadRequestSchema,
  TaxAutomationClient,
...
export async function POST(request: NextRequest) {
  ...
  const parsed = AccountantReviewQueueReadRequestSchema.safeParse(
    await readBoundedJson(request, MAX_REQUEST_BYTES).catch(() => null),
  );
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Invalid accountant review queue request" },
      { status: 400 },
    );
  }
  ...
  if (
    ...
    !grant.scopes.includes("tax.accountant.review_queue.read")
  ) {
    return NextResponse.json(
  ...
  try {
    const data = await client().getAccountantReviewQueue(

...
async function ensureBufiInstallation(accountLogin: string): Promise<void> {
  ...
  try {
    const resp = await appOctokit.rest.apps.getOrgInstallation({
      org: accountLogin,
    });
    install = {
      id: resp.data.id,
    ...
    try {
      const resp = await appOctokit.rest.apps.getUserInstallation({
        username: accountLogin,
      });
      install = {
        id: resp.data.id,
        accountType: "User",
        repositorySelection: resp.data.repository_selection as
          | "all"
          | "selected",
        htmlUrl: resp.data.html_url ?? null,
      };
...
function isValidBody(value: unknown): value is DispatchRequestBody {
  ...
  if (v.callback !== undefined) {
    ...
    if (typeof cb.url !== "string" || typeof cb.secret !== "string")
      return false;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Bot user provisioning failed" },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    console.error(
      "[bufi-dispatch] createSessionWithInitialChat failed:",
      error,
    );
    return NextResponse.json(

...
mock.module("@/lib/operating-packs/tax-authority-approval", () => ({
  HumanFacturaEAuthorityApprovalSchema: z
    .object({
    ...
    .strict(),
  registerHumanFacturaEAuthorityApproval: async (
...
function request(value: unknown = body) {
  return new Request(
    "https://open-agents.test/api/bufi/tax-authority-approval",
    {
      method: "POST",
      headers: {
...
describe("BUFI Tax authority approval ingress", () => {
  ...
  test("requires the dedicated authority scope and exact frozen-hash acknowledgement", async () => {
    ...
    expect(
      (await POST(request({ ...body, acknowledgement: "yes" }))).status,
    ).toBe(400);

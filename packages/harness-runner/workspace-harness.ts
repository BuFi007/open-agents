...
function validateCapability(
  capability: HarnessMcpCapability,
): HarnessMcpCapability {
  requireId("capability name", capability.name);
  if (!capability.scopes.length)
    throw new Error("workspace harness capability requires scopes");
  if (!capability.allowedOperations.length)
    throw new Error("workspace harness capability requires operations");
  for (const scope of capability.scopes) requireId("scope", scope);
  for (const operation of capability.allowedOperations)
    requireId("operation", operation);
  return {
    ...capability,
    scopes: [...new Set(capability.scopes)],
    allowedOperations: [...new Set(capability.allowedOperations)],
  };
}

export function createWorkspaceHarness(
  input: WorkspaceHarness,
): WorkspaceHarness {
  ...
  if (
    capabilities.some((capability) => capability.server === "bufi-hyper") &&
    input.connectionState !== "connected"
  )
    throw new Error("bufi-hyper MCP requires a connected harness");
  return { ...input, capabilities };
}

export function createMcpInvocationEvent(
  harness: WorkspaceHarness,
  input: { capability: string; operation: string; atMs: number },
): HarnessMcpInvocationEvent {
  const valid = createWorkspaceHarness(harness);
  const capability = valid.capabilities.find(
    (candidate) => candidate.name === input.capability,
  );
  if (!capability)
    throw new Error("MCP capability is not granted to this workspace harness");
  if (!capability.allowedOperations.includes(input.operation))
    throw new Error("MCP operation is not granted to this workspace harness");

...
describe("tenant-safe knowledge store", () => {
  it("resolves concurrent-style duplicate keys idempotently and isolates tenants", async () => {
    ...
    expect((await store.page("a")).items).toHaveLength(1);
    expect((await store.page("b")).items).toHaveLength(1);
  });
  it("uses bounded cursors instead of unbounded graph downloads", async () => {
    ...
    for (let i = 0; i < 3; i++)
      await store.resolveOrCreate({
        workspaceId: "a",
        externalKey: String(i),
        kind: "entity",
        name: String(i),
      });
    const first = await store.page("a", undefined, 2);
    const second = await store.page("a", first.nextCursor, 2);

...
describe("knowledge steward change sets", () => {
  it("auto-commits deterministic connector facts with evidence", () => {
    const changeSet = createKnowledgeChangeSet({
      ...
      changes: [{ targetId: "entity_1", operation: "update", field: "legalName", nextValue: "Acme" }],
      eveTraceId: "trace_1",
    });
  ...
  it("routes ambiguous identity and destructive changes to review", () => {
    const changeSet = createKnowledgeChangeSet({
      ...
      changes: [{ targetId: "entity_1", operation: "merge", destructive: true, nextValue: "entity_2" }],
      eveTraceId: "trace_1",
    });
    expect(changeSet.decision).toBe("needs-review");
    const approved = applyStewardDecision(changeSet, { decision: "approved", approverId: "user_1", reason: "same provider id", appliedGraphVersion: "graph_2" });
  ...
  it("prevents null enrichment from overwriting trusted fields and records undo", () => {
    expect(() => createKnowledgeChangeSet({
      id: "cs_3",
      workspaceId: "ws_1",
      version: 1,
      ...
      changes: [{ targetId: "entity_1", operation: "update", field: "website", nextValue: null }],
      eveTraceId: "trace_1",
    ...
    const reviewed = createKnowledgeChangeSet({
      ...
      changes: [{ targetId: "entity_1", operation: "split", destructive: true }],
      eveTraceId: "trace_1",
    });
    expect(applyStewardDecision(reviewed, { decision: "undone", approverId: "user_1", reason: "bad split", undoChangeSetId: "cs_undo_1" }).undoChangeSetId).toBe("cs_undo_1");
  });
});

...
describe("Expo workflow inbox contract", () => {
  it("builds mobile status cards, approval actions, notifications, and deep links", () => {
    ...
    expect(inbox.conversationContext).toEqual({
      teamId: "team_1",
      harnessId: "hermes",
    ...
    expect(inbox.cards[0]?.deepLinks.map((link) => link.kind)).toContain(
      "workflow",
    );
    expect(inbox.cards[0]?.deepLinks.map((link) => link.kind)).toContain(
      "wallet-intent",
    ...
    expect(inbox.agentWallet).toEqual({
      availableTools: 1,
      approvalRequired: 1,
      workflowSteps: 7,
    });
  ...
  it("marks workflows completed without client-side orchestration", () => {
    const inbox = buildExpoWorkflowInbox({
      ...input,
      nodes: [

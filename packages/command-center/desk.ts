...
export type OperationConsoleLine = {
  id: string;
  atMs: number;
  kind: "intent" | "tool" | "workflow" | "trace";
  label: string;
  status: "info" | "success" | "warning" | "error";
};
export type AgentWalletToolView = AgentWalletToolContract & {
  available: boolean;
  status: "idle" | "invoked" | "approval-required";
...
export type DeskCommandCenter = {
  ...
  harness: Pick<
    WorkspaceHarness,
    "harnessId" | "connectionState" | "sandboxRef" | "capabilities"
  >;
  approvals: readonly ApprovalItem[];
  ...
  agentWallet: {
    tools: readonly AgentWalletToolView[];
    workflow: typeof CIRCLE_AGENT_WALLET_WORKFLOW;
  };
  widgets: readonly {
    id: string;
    kind: "entity-graph" | "workflow" | "agent-roster" | "saved-query";
    label: string;
  }[];
};
...
function traceToConsole(event: TraceEvent): OperationConsoleLine {
  const status = event.type.endsWith("completed")
    ? "success"
    : event.type.endsWith("blocked") || event.type.endsWith("cancelled")
      ? "warning"
...
function buildAgentWalletTools(
  input: CommandCenterInput,
): AgentWalletToolView[] {
  const capabilities = new Set(
    input.harness.capabilities.map((capability) => capability.name),
  );
  return CIRCLE_AGENT_WALLET_TOOLS.map((tool) => {
    const latestTrace = [...input.traces]
      .toReversed()
      .find((trace) => trace.toolName === tool.name);
    const pendingApproval = input.approvals.some(
      (approval) =>
        approval.status === "pending" && approval.capability === tool.name,
    );
    return {
      ...tool,
      available: capabilities.has(tool.name),
      status: pendingApproval
        ...
        : latestTrace
          ...
          : "idle",
      ...(latestTrace
        ? { latestTraceId: latestTrace.id, latestSummary: latestTrace.summary }
        : {}),
    };
  });
}

export function buildDeskCommandCenter(
  input: CommandCenterInput,
): DeskCommandCenter {
  ...
  for (const node of input.nodes) {
    ...
    if (nodeIds.has(node.id))
      throw new Error(`duplicate workflow node: ${node.id}`);
    nodeIds.add(node.id);
  }
  for (const edge of input.edges) {
    if (!nodeIds.has(edge.from) || !nodeIds.has(edge.to))
      throw new Error("workflow edge references unknown node");
  }
  const agentCards = input.nodes.map((node) => ({
    agentId: node.agentId,
    label: node.label,
    status: node.status,
  }));
  const consoleLines = input.traces
    .map(traceToConsole)
  ...
  return {
    ...
    harness: {
      ...
      connectionState: input.harness.connectionState,
      sandboxRef: input.harness.sandboxRef,
      capabilities: input.harness.capabilities,
    },
    ...
    agentWallet: {
      tools: buildAgentWalletTools(input),
      workflow: CIRCLE_AGENT_WALLET_WORKFLOW,
    },
    widgets: [

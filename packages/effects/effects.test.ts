...
describe("durable ERP effect commands", () => {
  ...
  it("normalizes provider aliases and blocks retries while an export is ambiguous", async () => {
    const store = createEffectStore();
    const command = await store.upsert(
      createExportPayableCommand({
        workspaceId: "ws_1",
        billId: "bill_1",
        provider: "conta-azul",
    ...
    const ambiguous = await store.record(command.commandId, {
      atMs: 2000,
      status: "ambiguous",
    ...
    await expect(store.claim(command.commandId, 3000)).rejects.toThrow(
      "requires reconciliation",
    );
    const confirmed = await store.record(command.commandId, {
      atMs: 4000,
      status: "confirmed",
      providerReference: "erp_bill_1",
      evidenceHash: "hash_2",
    });
    expect(confirmed.status).toBe("confirmed");
    expect((await store.claim(command.commandId, 5000)).status).toBe(
      "confirmed",
    );
  });

/**
 * open-agents/arize-phoenix/_fetch — Cloudflare-bot-challenge-resilient
 ...
 * we requested JSON and got `200 + text/html`, we hit the bot
...
function curlFetch(
  ...
  return new Promise<PhoenixResponse>((resolve) => {
    ...
    proc.on("error", (err) => {
      resolve(
        makeResponse(false, 0, "error", `curl spawn failed: ${err.message}`),
      );
    });
    proc.on("close", (exitCode) => {
      if (exitCode !== 0 && !stdout) {
        resolve(
          makeResponse(
            ...
            0,
            "error",
            `curl exit ${exitCode}: ${stderr.slice(0, 400)}`,
          ),
        );
        return;
      }
      ...
      resolve(
        makeResponse(status >= 200 && status < 300, status, "curl", text),
      );
    });
    if (init.body !== undefined) {

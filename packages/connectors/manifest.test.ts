...
describe("connector manifest contract", () => {
  it("requires explicit account selection unless one default exists", () => {
    ...
    ).toThrow("account selection is required");
  });

  it("validates Pipedream deployment and compiles bounded queue leaf jobs", () => {
    expect(validateConnectorManifest(manifest).capabilities).toContain(
      "knowledge-ingestion",
    );
    ...
    expect(
      jobs.every(
        (job) =>
          job.workspaceId === "ws_1" &&
          job.idempotencyKey.includes("qbo.invoice.v1"),
      ),
    ).toBe(true);
  });
});
